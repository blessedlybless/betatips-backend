...
router.get('/users', auth, isAdmin, async (req, res) => {
  try {
    ...
    console.log(`🚫 Blocked users: ${users.filter(u => !u.isActive).length}`);
...
router.patch('/users/:userId/status', auth, isAdmin, async (req, res) => {
  try {
    ...
    console.log(`🔄 Updating user status for ${userId}: ${isActive ? 'ACTIVATE' : 'BLOCK'}`);
    ...
    console.log(`✅ User ${updatedUser.username} is now ${isActive ? 'ACTIVE' : 'BLOCKED'}`);
